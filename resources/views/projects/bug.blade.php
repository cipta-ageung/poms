    ...
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item"><a href="{{ route('projects.show',$project->id) }}">{{$project->name}}</a></div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered dataTable" id="dataTable">
                                ...
                                <tr>
                                    ...
                                    <th> {{__('Created By')}}</th>
                                    <th> {{__('Date')}}</th>
                                    <th class="text-right" width="5%"> {{__('Action')}}</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach ($bugs as $bug)
                                    <tr>
                                        ...
                                        <td>{{ !empty($bug->createdBy)?$bug->createdBy->name:'' }}</td>
                                        <td>{{ \Auth::user()->dateFormat($bug->created_at) }}</td>
                                        <td class="action text-right">
                                            @can('edit bug report')
                                                <a href="#" data-url="{{ route('task.bug.edit',[$project->id,$bug->id]) }}" data-ajax-popup="true" data-title="{{__('Edit Bug')}}" data-toggle="tooltip" data-original-title="{{__('Edit')}}">
